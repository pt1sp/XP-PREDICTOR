...
function parseVsDetail(rawJson: string): any {
  try {
    const payload = JSON.parse(rawJson) as any;
    return payload?.data?.vsHistoryDetail ?? payload?.vsHistoryDetail ?? payload;
  } catch {
    return null;
  }
}

function formatVsDetail(parsedVs: any): FormattedMatch | null {
  ...
  const awards = Array.isArray(parsedVs?.awards)
    ? parsedVs.awards.map((a: any) => String(a?.name ?? "").trim()).filter(Boolean)
    : [];
